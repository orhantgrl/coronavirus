...
class ApiProvider {
  ...
  Future<CountriesData> fetchCountriesData() async {
    final response = await _request(url: _countriesDataUrl);
  ...
  Future<TotalData> fetchTotalData() async {
    final response = await _request(url: _totalDataUrl);
  ...
  _request({@required String url}) async {
    Client client = Client();
    return await client.get(url, headers: {
      HttpHeaders.contentTypeHeader: 'application/json',
      HttpHeaders.authorizationHeader: await _getApplicationToken()
    });
  }

  Future<String> _getApplicationToken() async {
    final parsedJson =
